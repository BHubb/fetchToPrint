...
if [ "$files" != "0" ]; then # If the number of pdf's is greater then 0 then;
	lp -d $CUPS_PRINTER -h $CUPS_SERVER $ATTACHMENT_DIR/*.$ATTACHMENT_TYPE >> $CUPS_LOG # Prints all $ATTACHEMNT_TYPE in $ATTACHMENT_DIR to $CUPS_SERVER
	echo $(date +'%Y-%m-%d %T') >> $FETCHTOPRINT_LOG # Prints out date and time a print job was done
	find $ATTACHMENT_DIR/ -name *.$ATTACHMENT_TYPE | awk -F'/' '{ print "\t"$NF }' >> $FETCHTOPRINT_LOG # Prints only the $ATTACHMENT_TYPE name to the log
